...
@extends('layouts.main') 
    @section('container') 
        ...
        {{-- mengecek apa ada postingan atau tidak --}}
        @if ($posts->count())  
            <div class="card mb-3">
            ...
            <div class="row">
                @foreach ($posts->skip(1) as $post) 
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            {{-- px = position kiri kanan sedangkan py = atas bawah --}}
                            <div class="position-absolute bg-dark px-3 py-2" style="background-color: rgba(0, 0, 0, 0.7)"> 
                                <a href="/categories/{{ $post->category->slug }}" class="text-decoration-none text-white">{{ $post->category->name }}
                            </a></div>
